...
export default function Form(props) {
  ...
  function getDataApi() {
    fetch("https://strapi-store-server.onrender.com/api/products")
      .then((res) => res.json())
      .then((data) => {
        setInfo(data.data), props.setShow(false);
      })
  ...
  async function search(e) {
    ...
    try {
      const response = await fetch(
        `https://strapi-store-server.onrender.com/api/products?search=${productRef.current.value}&category=${categoryRef.current.value}&company=${companyRef.current.value}&order=${idRef.current.value}&price=${inputValue}000`
      );
      if (!response.ok) {
